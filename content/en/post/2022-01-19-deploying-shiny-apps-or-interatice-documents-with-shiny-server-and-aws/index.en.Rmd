---
title: Deploying Shiny Apps or Interatice Documents with Shiny Server and AWS
author: Yang Wu
date: '2022-01-19'
slug: deploying-shiny-apps-or-interatice-documents-with-shiny-server-and-aws
categories:
  - R
  - AWS
tags:
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; If you are working for an organization that uses AWS EC2, the chances are that your data team or IT department may already have an EC2 instance running. In that case, I would consult your cloud manager or supervisor or whoever manages your organization's AWS account regarding the following:
...
* You would need to connect to EC2 via a Secure Shell (SSH) using a Command Line Interface (CLI), and so you need to obtain the AWS EC2 Key `.pem` private key file.
...
Because many tutorials and resources online are based on the Ubuntu Server AMI, we will use this as well. Not only does this option have the [free tier](https://aws.amazon.com/free/) option but, based on my experience, it also could save us a lot of pain in having to deal with system requirements later on. The Ubuntu 20.04.1 LTS (at the time of writing this post) is a well documented operating system with a large user base and so trouble shooting is relatively easier in my experience compared to an AMI such as the Amazon Linux 1 or 2 AMI, which are based on Red Hat Enterprise Linux (RHEL). If you work for an organization, you may not be able to choose which AMI to use. But the steps that follow should theoretically work with other AMI (for instance, we use the Amazon Linux 1 AMI at our organization); just note that you may run into problems installing the required system libraries and packages needed for deployment and even for R packages.

### Choose an Instance Type

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; We will choose `t2.micro`, which is free tier eligible. Depending on your needs for computing resources (for instance, installing R packages with complied code), you may run out of memory with 1 CPU and 1GB of RAM, therefore, you could also consider other instance types. I recommend reading the following [article](https://www.qa.com/about-qa/our-thinking/understanding-t2-and-t3-cpu-credits-in-aws/) to better understand the differences between instance types.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; An [elastic IP address](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/elastic-ip-addresses-eip.html) is different than your EC2 instance's Public IPv4 address; in short, an Elastic IP address is allocated to your AWS account, and is yours until you release it. Therefore, this IP address can be reused for your EC2 instances. The re-usability of our IP may be useful when we want to upgrade or downgrade our EC2 instance type. We can simply associate the elastic IP address to the new server. Or, if something bad happens to our EC2 instance for whatever reason, an elastic IP address allows us to mask the failure of an instance or software by rapidly remapping the address to another instance in our account.

<img src="images/elastic_ip.png" width="750" height="600">

Next, select the `Action` drop down menu in the top right corner and choose `Associate Elastic IP address`. From now on, every time we make changes to our EC2 instance, we can simply re-associate this IP address to our new instance. 
...
* Open your terminal, navigate to the location of your `.pem` key:
...
* Next, run the following command to ensure that your key is not publicly viewable:
...
If this is your first time connecting to your EC2 instance, you may receive an `Are you sure you want to continue connecting (yes/no/[fingerprint])?` prompt. Entering `yes` should successfully connect you to your EC2 instance: 
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; This particular step and many of the steps that follow are places where the operating system will begin to matter. The following commands are meant to work with the Ubuntu OS (a Debian-based Linux distribution). For instance, the [Advanced package tool](https://www.vultr.com/docs/using-apt-utilities-on-ubuntu/), or APT, used to handle the installation and removal of software is developed for Ubuntu/Debian system software packages. For Red Hat-based Linux systems such as the official Amazon Linux AMI 1 or 2, the [Yellowdog Updater, Modified](https://www.redhat.com/sysadmin/how-manage-packages), or YUM, package-management utility is used. In addition, R packages can depend on software external to the R ecosystem. On Ubuntu, for instance, in order to install the `curl` R package, you must install the system library first via `apt-get install libcurl`. Resolving system dependency issues can be painful at times, especially if you are using a different operating system (AMI). The one effective way for troubleshooting based on my own experience is simply Google searching for system dependencies on an ad-hoc basis (after an error is thrown, for instance, when you try to install an R package). If you are lucky, you won’t be the first to crash because of a missing system library. If you are using another AMI that runs another operating system, you must also figure out how to install the library.      
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; The difference between `apt-get` and `apt` is that the former is an older version with more command options while “apt” is a newer, more user-friendly command with fewer options. To understand these shell commands, I found that [explainshell.com](https://explainshell.com/) (its github repo can be found [here](https://github.com/idank/explainshell])) is extremely useful. Other resources are:
...
If `nginx` is installed successfully, you should see the following page by entering your [Public IPv4 address](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-instance-addressing.html) (obtained from `Instance summary`) into your web browser:
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; If you are using an operating such as the Amazon Linux AMI 1, the stable version of R in the default repository is, at the time of writing this post, 3.x; therefore, this may or may not be troublesome depending on the R packages you need to install.
...
**Note**: As mentioned earlier, with the `t2.micro` instance type, you may simply run out of memory installing some R packages with compiled code (`Rcpp`, `RcppArmadillo`). When this happens, the installation process would seemingly be stuck in an never-ending process that would need to be killed.

### Install R packages (User Library)

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Alternatively, there is also the option to install add-on R packages in a [user-level library](https://r-pkgs.org/package-structure-state.html#library), which may be appealing for many reasons. We will return to this once we set up the user credentials for Rstudio server.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; The results of `ls` will differ, sometimes substantially, depending on the AMI (and the operating system) that you are using. For instance, on Ubuntu, the default installation of `nginx` will create a `sites-avalable` and a `sites-enabled` directory. On RedHat/CentOS/Fedora, the default installation of `nginx` does not include such directories. For those operating systems, the default place to store the configuration files is the following directory `/etc/nginx/conf.d/*.conf`. Then, in the `/etc/nginx/nginx.conf` file, add the `include /etc/nginx/conf.d/*.conf;` directive in the http block to tell `nginx` to pull in any files in the `conf.d` directory that has the extension `.conf`. We are on Ubuntu, and so the following set up steps are needed:

* Having navigated to the `/etc/nginx` directory, we should see the following sub-directories:
...
* Add the following location block in addition to the existing location block with the shortest prefix "/". However, we will use the name of the sub-directory within `/srv/shiny-server/` (for me, this is `/portfolio_dashboard`) as `location_match` so that only the application located in that sub-directory is password protected. This ensures that not all of your applications use the same password. Once you create more than one of such sub-directories under `/srv/shiny-server/` to host multiple applications, you have the flexibility to choose which ones are public and password protected with this set up.
...
More details on `nginx` location blocks can be found [here](http://nginx.org/en/docs/beginners_guide.html) under section "Serving Static Content". For more on `nginx` server and location block selection algorithms, see the following [article](https://www.digitalocean.com/community/tutorials/understanding-nginx-server-and-location-block-selection-algorithms). 
...
server {
    ...
    # For portfolio_dashboard
    location /portfolio_dashboard {
        proxy_pass http://127.0.0.1:3838/;
        proxy_redirect http://127.0.0.1:3838/ $scheme://$host/;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
...
The hyperlinks embedded through out the post are some of the resources that have helped me improve my understanding (however imperfect) of the deployment process, especially through peaking *under the hood* of things. In addition, I also found the following links extremely useful.  
